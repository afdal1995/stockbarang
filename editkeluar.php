...
try {
    // Ambil qty lama dari tabel partkeluar
    $sql_get_old_qty = "SELECT qty FROM partkeluar WHERE no='$id'";
    ...
    // Update tabel partkeluar
    $sql_partkeluar = "UPDATE partkeluar SET
        document='$document',
    ...
    if (!$conn->query($sql_partkeluar)) {
        throw new Exception("Error updating partkeluar: " . $conn->error);
    }
    ...
    if ($oldPartnumber !== $newPartnumber) {
        ...
        // Kembalikan kuantitas lama ke partnumber lama
        ...
        // Kurangi kuantitas baru pada partnumber baru
        ...
        // Jika partnumber sama, hanya update kuantitas
        $sql_update_new_partnumber = "UPDATE datastock SET qty = qty - ($new_qty - $old_qty) WHERE partnumber='$newPartnumber'";
        if (!$conn->query($sql_update_new_partnumber)) {
            throw new Exception("Error updating new partnumber qty: " . $conn->error);
        }
    }
