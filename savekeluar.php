...
try {
    ...
    foreach ($data as $item) {
        ...
        $sql_check = $conn->prepare("SELECT COUNT(*), qty FROM datastock WHERE partnumber = ?");
        $sql_check->bind_param("s", $partnumber);
        $sql_check->execute();
        $sql_check->bind_result($count, $current_qty);
        $sql_check->fetch();
        $sql_check->close();
        ...
        if ($current_qty < $item['qty']) {
            $all_data_valid = false;
            $error_message = 'Kuantitas untuk partnumber ' . $partnumber . ' tidak mencukupi.';
            break;
        }
    }
    ...
    // Menyiapkan statement untuk memasukkan data ke tabel partkeluar
    $stmt = $conn->prepare("INSERT INTO partkeluar (document, partnumber, oem, description, location, qty) VALUES (?, ?, ?, ?, ?, ?)");
    if ($stmt === false) {
    ...
    foreach ($data as $item) {
        ...
        // Mengurangi kuantitas di tabel datastock
        $partnumber = $item['partnumber'];
        $qty = $item['qty'];
        
        // Menyiapkan statement untuk mengurangi kuantitas
    ...
    $stmt_select = $conn->prepare("SELECT * FROM partkeluar WHERE partnumber IN ($partnumber_placeholders)");
