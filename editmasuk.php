...
try {
    // Ambil qty lama dari tabel partmasuk
    $sql_get_old_qty = "SELECT qty FROM partmasuk WHERE no='$id'";
    ...
    // Update tabel partmasuk
    $sql_partmasuk = "UPDATE partmasuk SET
        document='$document',
    ...
    if (!$conn->query($sql_partmasuk)) {
        throw new Exception("Error updating partmasuk: " . $conn->error);
    }
    ...
    // Hitung selisih qty yang diupdate
    ...
    if ($oldPartnumber !== $newPartnumber) {
        ...
        // Jika partnumber sama, hanya update qty
        $sql_update_same_partnumber = "UPDATE datastock SET qty = qty + $qty_difference WHERE partnumber='$newPartnumber'";
        if (!$conn->query($sql_update_same_partnumber)) {
            throw new Exception("Error updating qty for same partnumber: " . $conn->error);
        }
    }
