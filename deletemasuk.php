...
if (!empty($no)) {
    // Menyiapkan statement SQL untuk mengambil data dari partmasuk
    $stmt = $mysqli->prepare('SELECT partnumber, qty FROM partmasuk WHERE no = ?');
    ...
    if ($stmt) {
        ...
        if ($partnumber) {
            $stmtDelete = $mysqli->prepare('DELETE FROM partmasuk WHERE no = ?');
            if ($stmtDelete) {
            ...
            echo json_encode(['status' => 'error', 'message' => 'Data tidak ditemukan di partmasuk.']);
        }
    } else {
