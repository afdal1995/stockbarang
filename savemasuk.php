...
try {
    ...
    // Menyiapkan statement untuk memasukkan data ke tabel partmasuk
    $stmt = $conn->prepare("INSERT INTO partmasuk (document, partnumber, oem, description, location, qty) VALUES (?, ?, ?, ?, ?, ?)");
    if ($stmt === false) {
    ...
    $stmt_select = $conn->prepare("SELECT * FROM partmasuk WHERE partnumber IN ($partnumber_placeholders)");
